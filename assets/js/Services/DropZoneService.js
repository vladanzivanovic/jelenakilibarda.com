...
export default (() => {
    ...
    Private.addFiles = (dropZoneElm, files) => {
        ...
        Private.mapper.loader.removeClass('d-none');
        ...
        Promise.all(requests)
            .then(function (response) {
                ...
                Loader.removeLoader();
                Private.mapper.loader.addClass('d-none');
            })
            .catch(error => {
                Loader.removeLoader();
                Private.enableDropzone();
    ...
    Private.processNewFile = function (file, counter, index) {
        ...
        return $.ajax({
            type: 'POST',
            url: Routing.generate('image_resize_on_fly_api'),
            data: data,
            contentType: false,
            processData: false,
        });
    ...
    Private.checkMaxFiles = (maxFiles, files) => {
        ...
        if (count > maxFiles) {
            Private.notify.show('error', Translator.trans('files.limit_reached', null, 'validators', LOCALE));
