...
class ShippingHandler {
    ...
    save() {
        let urlRoute = Routing.generate('admin.add_shipping_api');
        let type = 'POST';
        ...
        if (IS_EDIT) {
            urlRoute = Routing.generate('admin.edit_shipping_api', {id: ID});
            type = 'PUT';
        }
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: response => {
                AppHelperService.redirect(Routing.generate('admin.shipping_page'));
            },
            error: error => {
    ...
    remove(id) {
        ...
        $.ajax({
            type: 'DELETE',
            url: Routing.generate('admin.remove_shipping_api', {id}),
            dataType: 'json',
            success: () => {
    ...
    changeStatus(checkbox, id, status) {
        $.ajax({
            type: 'PATCH',
            'url': Routing.generate('admin.api_shipping_change_status', {id, status}),
            dataType: 'json',
            success: (response) => {
