...
export default (() => {
    ...
    Public.init = () => {
        Private.tableRef.DataTable( {
            serverSide: true,
            ajax: {
                url: Routing.generate('admin.get_shipping_list'),
                type: 'POST'
            },
            columns: [
                { data: 'id', name: 'id', title: 'Id' },
                { data: 'title_hr', name: 'title_hr', title: 'Naziv'},
                { data: 'price', name: 'price', title: 'Cena'},
                ...
                { data: 'id', orderable: false, render: function (data, type, row, meta) {
                    const editLink = CAN_EDIT ? `<a class="btn btn-outline-primary" href="${Routing.generate('admin.edit_shipping_page', {id: data})}">Izmeni</a> ` : '';
