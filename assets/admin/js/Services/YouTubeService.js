...
import ArrayFilters from "../../../js/Helper/ArrayFilters";
...
export default (() => {
    ...
    Public.createVideo = (url = null) => {
        ...
        if (!youTubeId) {
            Private.notify.show('warning', MESSAGE.ERROR.ID_REQUIRED, true);
        }
        else if(!Public.checkExistByProp('YouTubeId', youTubeId)){
            Private.notify.show('error', MESSAGE.ERROR.YOUTUBE_EXIST, true);
        }else {
            Private.getYouTube(youTubeId)
                ...
                .fail(error => {
                    Private.notify.show('error', MESSAGE.ERROR.YOUTUBE_URL_NOT_VALID, true);
                })
        }
    };
